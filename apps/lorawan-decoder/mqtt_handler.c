/**
 * @file
 * @brief MQTT handler implementation for LoRaWAN MQTT Decoder Service
    ...
    const struct mosquitto_message *msg)
{
    ...
    LOG_DEBUG("Received message on topic: %s (len=%d)", 
    ...
    bool retain)
{
    ...
    rc = mosquitto_publish(
        g_mosq, NULL, topic, (int)payload_len, payload, qos, retain);
    if (rc != MOSQ_ERR_SUCCESS) {
        LOG_ERROR("Failed to publish to topic: %s (error=%d)", topic, rc);
        return false;
    }

    LOG_DEBUG("Published to topic: %s (len=%zu)", topic, payload_len);
    return true;
}
